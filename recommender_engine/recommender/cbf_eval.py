...
class ContentBasedEvaluator:
    ...
    def __init__(self, db_client: Neo4jClient):
        ...
        self.test_df = None
        self.all_items = None
        self.item_names_map = {}  # Map of (item_id, item_type) -> name
    ...
    def load_data(self):
        ...
        self.all_items = self.interactions_df[[
            'item', 'item_type']].drop_duplicates().values.tolist()

        # Create item names map for reference
        for _, row in self.interactions_df.iterrows():
            self.item_names_map[(row['item'], row['item_type'])] = row['user']

        print(
            f"Loaded {len(self.interactions_df)} interactions for {self.interactions_df['user'].nunique()} users")
    ...
    def train_test_split(self, test_size=0.2, random_state=42):
        ...
        self.test_df = pd.concat(test_dfs) if test_dfs else pd.DataFrame(
    ...
    def evaluate_single_user(self, user_id: str, new_user: bool, top_n: int = 10,
        ...
        actual_items = set(tuple(x)
                           for x in user_test[['item', 'item_type']].values)
        ...
        try:
            ...
            print(
                f"Error initializing recommender for user {user_id}: {str(e)}")
            return None

        metrics = {}

        # Get recommendations for each item_type and overall
        for type_filter in ['Event', 'Destination', None]: #Trip
            ...
            try:
                ...
                for _, row in rec_df.iterrows():
                    # Find the item ID for this name and item_type
                    for (item_id, item_type), name in self.item_names_map.items():
                        if name == row['name'] and item_type == row['item_type']:
                            recommended_items.append((item_id, item_type))
                            break

                # Calculate precision and recall
                recommended_set = set(recommended_items)
                relevant_and_recommended = recommended_set.intersection(
                    actual_items)

                precision = len(relevant_and_recommended) / \
                    len(recommended_set) if recommended_set else 0
                recall = len(relevant_and_recommended) / \
                    len(actual_items) if actual_items else 0
                ...
                print(
                    f"Error getting recommendations for user {user_id}, item_type {type_filter}: {str(e)}")
                metrics[f"{type_name}_precision"] = 0
                metrics[f"{type_name}_recall"] = 0
                metrics[f"{type_name}_ndcg"] = 0
    ...
    def _calculate_ndcg(self, recommended_items: List[Tuple], actual_items: Set[Tuple]) -> float:
        ...
        relevance = [
            1 if item in actual_items else 0 for item in recommended_items]
    ...
    def diversity_analysis(self, test_users: List[str], top_n: int = 10,
            ...
            try:
                ...
                if not rec_df.empty:
                    ...
                    for _, row in rec_df.iterrows():
                        ...
                        item_coverage.add((row['name'], row['item_type']))
                ...
                print(
                    f"Error analyzing diversity for user {user_id}: {str(e)}")
                continue
        ...
        inter_user_diversity = self._calculate_inter_user_diversity(
            all_recommendations)

        # Intra-list diversity: average tag diversity within each recommendation list
        intra_list_diversity = self._calculate_intra_list_diversity(
            all_recommendations)

        # Calculate coverage ratios
        # For item coverage, we need the total number of items in the dataset
        total_items = len(self.all_items)
        item_coverage_ratio = len(item_coverage) / \
        ...
        # Placeholder (can't calculate without knowing total tags)
        tag_coverage_ratio = 1.0
    ...
    def _calculate_inter_user_diversity(self, all_recommendations: List[List[Dict]]) -> float:
        ...
        # Convert recommendations to sets of (name, item_type) tuples for comparison
        rec_sets = []
        for user_recs in all_recommendations:
            rec_set = {(item['name'], item['item_type']) for item in user_recs}
...
if __name__ == "__main__":
    ...
    comparison_results = evaluator.compare_reranking_strategies(
        test_users=test_user_ids, top_n=10)
    print("\nReranking Strategy Comparison:")
    print(comparison_results)

    # Analyze diversity
    diversity_metrics = evaluator.diversity_analysis(
        test_users=test_user_ids, top_n=10)
    print("\nDiversity Metrics:")
